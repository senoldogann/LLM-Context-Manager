...
setup_wrapper() {
    ...
    info "Configuring MCP wrapper script..."
    
    # Update paths in wrapper
    sed -i.bak "s|cd .*|cd \"$INSTALL_DIR\"|" "$WRAPPER_FILE" 2>/dev/null || \
    sed -i '' "s|cd .*|cd \"$INSTALL_DIR\"|" "$WRAPPER_FILE"
    
    chmod +x "$WRAPPER_FILE"
    
    success "Wrapper script configured"
}
